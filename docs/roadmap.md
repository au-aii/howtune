...
- [ ] **生 ReactionEvent 保存で「本物の反応密度＋タグ」に（moat）** → [issue #112](https://github.com/engineer-guild-hackathon-2026-05/team-10/issues/112)
  - 設計 steering: `.steering/20260705-reaction-event-persistence/`（`reaction_sessions` スキーマ・段階案 P-a/b/ML/c・生理学的精査＋ML前提を統合・未決4点は確定済み）
  - ✅ **P-a 実装＋本番デプロイ＋バックエンド検証まで完了**（iOS 書き込み経路・route・`onReactionSessionWritten` トリガー・集計統合・rules。本番トリガー稼働）
  - ⬜ 残り＝**実機火入れ＋フォローアップ（self_report 配線 / P-b / P-ML / node20→22）** → [issue #9](https://github.com/au-aii/howtune/issues/9)
