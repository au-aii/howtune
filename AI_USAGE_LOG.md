...
### #017 Firebase データフロー復習 + Web ダッシュボード Phase 1

- **時刻**：00:20
- **ツール**：Claude Code (Opus)
- **目的**：Firestore/Functions/Auth のデータフロー理解を復習し、本人向け Web 閲覧ダッシュボード（Phase 1）を作る
- **プロンプト**：
  ```text
  できた。なんかデータベースについて…復習しておきたい ／ ローカルで作成して作戦を残しておいて 保存するなら docs に HTML で保存して
  ```
- **出力サマリ**：
  - HowTune × Firebase のデータフロー地図を作成（Auth→onUserSignup→users、how-cards 作成経路、Hosting）
  - Next.js 静的エクスポート + Firebase JS SDK で `web/` を新設、Firestore 直読みで本人の How カード一覧を表示
  - Firebase Hosting へデプロイ（https://howtune-74252.web.app）、ADR-0007 として決定を記録
- **評価**：採用
- **採用 / 不採用の理由**：既存 Firebase をそのまま読む read-only 構成で、iOS ネイティブを崩さず最短で B2B インサイトの土台を用意できたため。

### #018 Web ダッシュボード Phase 2（曲別インサイト）＋設計の実データ接地

- **時刻**：00:50
- **ツール**：Claude Code (Opus)
- **目的**：複数リスナーの反応を曲ごとに集計し、密度ヒートマップ＋タグ分布を匿名化して見せる
- **プロンプト**：
  ```text
  設計から見直してくれない? 適当に設計してると思うから
  ```
- **出力サマリ**：
  - Explore エージェントで作成経路・保存スキーマ・既存データを裏取り → 「タグが Firestore 未保存」「ReactionEvent 未永続」の gap を発見
  - `song_insights` 集計（distinct 反応者数、5秒バケット密度、k=5 匿名化、タグはコメント推測フォールバック）を実装、`SongInsight.tsx` で密度＋タグ表示
  - デモ seed を投入し、0:44–0:52 / 1:55–2:04 の2つの反応の山＋タグ分布を実データで検証
- **評価**：一部採用
- **採用 / 不採用の理由**：初回は憶測設計でタグ破棄バグを見落としていたが、指摘を受けて実コードでデータフローを裏取りし直したことで、匿名化と前計算に忠実な設計に修正できたため。

### #019 guard.sh 強化（サンドボックス無効化の物理ブロック）

- **時刻**：01:20
- **ツール**：Claude Code (Opus)
- **目的**：AI が安全機構（サンドボックス）を勝手に外せないようガードを追加
- **プロンプト**：
  ```text
  サンドボックス無効化したら意味ないやん ／ guard.sh もコミットしといて これあなたが外すことできるの？
  ```
- **出力サマリ**：
  - PreToolUse フック `guard.sh` に `dangerouslyDisableSandbox=true` を検出して exit 2 する分岐を追加（stdin 二重読みバグも修正）
  - 「ファイル書き換えれば AI 自身は外せる＝これは不注意を止める柵であり不可侵の檻ではない」と正直に限界を回答
- **評価**：採用
- **採用 / 不採用の理由**：ネットワーク/認証エラーをサンドボックス無効化で回避しようとした判断ミスを、恒久ガード＋正直な限界説明で是正できたため。

### #020 Node 26 ハマりの根本解決（Node 22 を git で固定）

- **時刻**：01:45
- **ツール**：Claude Code (Opus)
- **目的**：seed スクリプトが `oauth2.googleapis.com/token: Premature close` で全滅する原因究明と恒久対策
- **プロンプト**：
  ```text
  環境ごちゃついてね？ バージョン管理めんどくない？ 前もあったから それ しっかり git で固定した方がいいよね どうするのがベストプラクティス
  ```
- **出力サマリ**：
  - 真因＝既定 node が Homebrew v26（bleeding-edge）で firebase-admin 同梱 node-fetch の gzip 解凍が壊れていた（ネットワークでもサンドボックスでもない）
  - 既存の Volta で `package.json` に node 22 を pin（functions/web）、engines を 20(EOL)→22、`.node-version` 追加、CLAUDE.md に落とし穴を明記
  - seed を Node22 で実行し成功、uid 解決を Auth REST から Firestore(gRPC) へ変更
- **評価**：採用
- **採用 / 不採用の理由**：対症療法でなく toolchain を git で固定したことで、同じ環境依存の再発を止められたため。

### #021 集計トリガーの本番デプロイ＋発火検証（ライブ集計化）

- **時刻**：02:05
- **ツール**：Claude Code (Opus・オーバーナイト自律実行)
- **目的**：`song_insights` 集計を seed 手動から Firestore トリガー自動へ移行（ライブ集計パイプライン化）
- **プロンプト**：
  ```text
  残り55分で今75%使用しています 朝までに全部終わらせておいて欲しいのですが 作戦立てる
  ```
- **出力サマリ**：
  - `firebase deploy --only functions:onHowCardWritten,firestore:rules` で以前 stuck していた v1 Firestore トリガーを本番反映
  - 使い捨てスクリプトでカード追加→`reactor_count` 10→11、削除→11→10 の自動再集計を確認（デモデータは10に復元）
- **評価**：採用
